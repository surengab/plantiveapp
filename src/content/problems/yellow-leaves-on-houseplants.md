---
title: 'Why Are My Plant Leaves Turning Yellow? 7 Causes and Fixes'
description: 'Yellow leaves on a houseplant have seven likely causes. Work through them in order of probability — starting with the soil test that settles it in ten seconds.'
heading: 'Why Are My Plant Leaves Turning Yellow?'
publishDate: 2026-07-10
updatedDate: 2026-08-18
keywords:
  - yellow leaves houseplant
  - why are my plant leaves turning yellow
  - plant leaves yellowing
  - overwatering yellow leaves
symptom: 'Leaves turning yellow'
severity: medium
quickAnswer: 'Overwatering causes yellow leaves far more often than anything else. Push a finger two inches into the soil: if it is damp, stop watering and let it dry out.'
causes:
  - name: 'Overwatering'
    likelihood: 'most likely'
    tell: 'Soil still damp several days after watering. Yellowing starts on lower and inner leaves, which feel soft rather than crisp. Sometimes a sour smell from the pot.'
    fix: 'Stop watering until the top two inches are dry. Check the pot actually drains and empty any saucer. If soil stays wet more than a week, repot into a mix with more perlite or bark.'
  - name: 'Underwatering'
    likelihood: 'common'
    tell: 'Soil bone dry and pulling away from the pot edge. Leaves yellow then crisp brown, and the whole plant may droop.'
    fix: 'Soak thoroughly until water runs from the drainage holes. If water runs straight through without wetting the soil, bottom-water by standing the pot in a tray for 20 minutes.'
  - name: 'Natural ageing'
    likelihood: 'common'
    tell: 'One or two of the oldest, lowest leaves yellow slowly while all new growth looks healthy.'
    fix: 'Nothing to fix. Snip the old leaf off at its base. Every plant retires old leaves to redirect energy into new ones.'
  - name: 'Not enough light'
    likelihood: 'common'
    tell: 'Yellowing paired with slow growth, long gaps between leaves, and stems leaning hard toward the window.'
    fix: 'Move closer to a window or add a grow light. Yellowing from low light develops gradually over weeks rather than suddenly.'
  - name: 'Nutrient deficiency'
    likelihood: 'less common'
    tell: 'Yellowing between green veins, or a general pale wash across many leaves. Common in plants unrepotted for two or more years.'
    fix: 'Feed with a balanced liquid fertiliser at half strength during the growing season, and refresh the top two inches of soil or repot entirely.'
  - name: 'Pests'
    likelihood: 'less common'
    tell: 'Fine stippling or speckled yellowing, webbing in leaf joints, sticky residue, or tiny moving specks on leaf undersides.'
    fix: 'Inspect undersides with a phone camera zoomed in. Shower the plant, then treat weekly with insecticidal soap or neem for three weeks.'
  - name: 'Shock from a recent change'
    likelihood: 'rare'
    tell: 'Yellowing began within two weeks of repotting, a move, or a sharp temperature change.'
    fix: 'Keep conditions stable and wait. Do not repot again or change the routine — most plants adjust within a month.'
affectedPlants:
  - pothos
  - monstera-deliciosa
  - peace-lily
  - zz-plant
  - spider-plant
faqs:
  - question: 'Should I cut off yellow leaves?'
    answer: 'Yes, once a leaf has turned substantially yellow it will not turn green again, and the plant is already withdrawing nutrients from it. Cut it off at the base of its stem with clean scissors. Removing it also makes it much easier to tell whether the problem is continuing, because any new yellowing is fresh information.'
  - question: 'Can a yellow leaf turn green again?'
    answer: 'Almost never. Chlorophyll loss in a leaf is generally irreversible, so the leaf stays yellow whatever you do. The exception is mild nutrient deficiency caught early, where feeding can partially restore colour. Judge your fix by whether new growth comes in healthy, not by whether old leaves recover.'
  - question: 'How do I tell overwatering from underwatering when both cause yellow leaves?'
    answer: 'Feel the soil two inches down, and feel the leaf. Overwatered plants have damp soil and soft, limp yellow leaves. Underwatered plants have bone-dry soil, often shrunk away from the pot edge, and yellow leaves that go crisp and papery. The soil test settles it in about ten seconds and is far more reliable than looking at the leaves alone.'
  - question: 'Is it normal for a plant to have a few yellow leaves?'
    answer: 'Yes. Every plant sheds its oldest leaves as it grows, and losing one or two lower leaves every few months while new growth continues is completely normal. Worry when several leaves yellow at once, when the yellowing is on new growth, or when it is accompanied by drooping or soft stems.'
featured: true
---

Yellow leaves are the most common houseplant complaint there is, and the frustrating part is that opposite problems produce nearly identical symptoms. Work through the causes in order — they are ranked by how often each actually turns out to be the answer.

## Start here: the soil test

Before anything else, push your index finger two inches into the soil.

- **Damp** → go to overwatering. This is the answer roughly six times out of ten.
- **Bone dry, soil shrunk from the pot edge** → go to underwatering.
- **Slightly moist, neither extreme** → water is probably not your problem; look at light, age and pests.

This single test eliminates most of the possibilities in ten seconds, and it is far more reliable than judging by leaf appearance.

## Where on the plant matters

The position of the yellowing narrows things down considerably:

| Where | Most likely cause |
| --- | --- |
| Oldest, lowest leaves only, one at a time | Natural ageing — normal |
| Lower and inner leaves, several at once | Overwatering |
| Whole plant evenly, pale and washed out | Nutrient deficiency or low light |
| Between the veins, veins stay green | Iron or magnesium deficiency |
| New growth at the tips | Serious root problem or nutrient lockout |
| Speckled or stippled patches | Pests |

## Overwatering, the usual culprit

More houseplants are killed by kindness than neglect. Constantly wet soil drives out the air that roots need, they suffocate and begin to rot, and a damaged root system cannot move water or nutrients — so the leaves yellow.

**Confirming it:** soil damp several days after watering, yellowing on lower and inner leaves, leaves soft rather than crisp, sometimes a sour smell.

**Fixing it:**

1. Stop watering. Let the top two inches dry completely.
2. Check the pot has drainage holes and that they are not blocked.
3. Empty the saucer or cachepot — never let the pot stand in water.
4. If the soil is still wet after a week, unpot and check the roots.
5. Repot into a chunkier mix with added perlite or bark if the soil is dense.

Healthy roots are firm and pale. Brown, soft, stringy roots that smell sour are [root rot](/problems/root-rot/) and need trimming out.

## Underwatering

Less common indoors but very common in plants that have become root-bound, where the root ball repels water and it runs straight down the sides of the pot.

**Confirming it:** soil bone dry and pulled away from the pot edge, leaves yellowing then going crisp and papery, whole plant possibly drooping.

**Fixing it:** water thoroughly until it drains from the bottom. If water runs straight through in seconds, bottom-water instead — stand the pot in a tray of water for 20 minutes and let it wick up until the surface darkens.

## When it is simply age

If your plant is putting out healthy new growth and one old bottom leaf has slowly gone yellow, nothing is wrong. Plants retire old leaves and recycle their nutrients into new ones. Snip it off and carry on.

The distinction that matters: **one leaf at a time, from the bottom, while new growth thrives** is ageing. **Several leaves at once, anywhere on the plant** is a problem.

## Nutrient deficiency

Suspect this in a plant that has been in the same soil for two or more years.

- **Yellow between green veins** on newer leaves → iron deficiency, often from soil pH being too high
- **Yellow between green veins** on older leaves → magnesium deficiency
- **Uniform pale yellow-green** across the whole plant → nitrogen deficiency

The fix for all three is the same at the household level: feed with a balanced liquid fertiliser at half strength through the growing season, and refresh the soil. Do not feed a plant you suspect of root rot — damaged roots cannot take up nutrients and the added salts make things worse.

## Checking for pests

Turn a few leaves over and look closely, ideally with your phone camera zoomed in.

- **Fine stippling with tiny webs** in leaf joints → [spider mites](/problems/spider-mites/)
- **White cottony clumps** in leaf axils → mealybugs
- **Sticky residue** on leaves or the surface below → scale or aphids
- **Silvery streaks with black specks** → thrips

Isolate any affected plant from its neighbours, shower it in the sink, and treat weekly with insecticidal soap for three weeks to catch successive hatches.

## After you fix it

Judge your diagnosis by **new growth**, not by the old leaves. Yellow leaves will not turn green again — but if the fix is right, the leaves that emerge over the following weeks will come in healthy and stay that way.
