---
title: 'Rubber Plant Care: Light, Watering and Leaf Drop Fixes'
description: 'Rubber plant (Ficus elastica) care: how often to water, the light it needs for burgundy colour, why leaves drop, and how to prune for a bushier shape.'
heading: 'Rubber Plant Care Guide'
publishDate: 2026-06-26
updatedDate: 2026-08-15
keywords:
  - rubber plant care
  - ficus elastica care
  - rubber plant dropping leaves
  - rubber tree plant care
  - how often to water rubber plant
commonName: 'Rubber Plant'
botanicalName: 'Ficus elastica'
alsoKnownAs:
  - Rubber tree
  - Rubber fig
  - Indian rubber bush
family: 'Moraceae'
origin: 'Northeast India, Nepal, Myanmar and Southeast Asia'
silhouette: round
hue: 350
difficulty: easy
petSafe: false
matureSize: '6-10 ft tall indoors'
growthRate: moderate
toxicity: 'Milky latex sap irritates skin and causes mouth irritation, drooling and vomiting in cats and dogs. Wear gloves when pruning.'
care:
  light:
    label: 'Bright indirect'
    detail: 'Bright indirect light keeps growth compact and deepens the burgundy tones of varieties like Ruby and Burgundy. Tolerates medium light with slower, leggier growth.'
  water:
    label: 'Every 7-14 days'
    detail: 'Water when the top 2 inches are dry. Rubber plants are noticeably more drought-tolerant than fiddle leaf figs.'
  humidity:
    label: '40-60%'
    detail: 'Average indoor humidity suits it. Wipe the large leaves monthly to remove dust rather than misting.'
  temperature:
    label: '60-80°F (15-27°C)'
    detail: 'Keep above 55°F (13°C) and away from cold drafts, which trigger sudden leaf drop.'
  soil:
    label: 'Well-draining potting mix'
    detail: 'Standard indoor mix with 20-30% perlite or bark for drainage.'
  fertilizer:
    label: 'Monthly in spring and summer'
    detail: 'Balanced liquid feed at half strength. Stop entirely from late autumn through winter.'
  repotting:
    label: 'Every 2-3 years'
    detail: 'Move up one pot size in spring. Keeping it slightly root-bound limits height if you do not want a tree.'
  propagation:
    label: 'Stem cuttings or air layering'
    detail: 'Take a cutting with two or three leaves, blot the sap, and root in water or damp mix. Air layering works better on thick woody stems.'
troubleshooting:
  - symptom: 'Lower leaves yellowing then dropping'
    cause: 'Overwatering, or a natural shed after a move'
    fix: 'Check that the soil dries down two inches between waterings. A few lower leaves after relocation is normal and stops once conditions stabilise.'
    problemSlug: 'yellow-leaves-on-houseplants'
  - symptom: 'Leaves dropping suddenly while still green'
    cause: 'Cold draft or sharp temperature change'
    fix: 'Move away from doors, windows and heating vents. Ficus species shed leaves in protest at environmental change.'
  - symptom: 'Burgundy variety turning plain green'
    cause: 'Not enough light'
    fix: 'Move closer to a bright window. The red pigments develop only in strong indirect light.'
  - symptom: 'Tall bare stem with leaves at the top only'
    cause: 'Natural apical growth habit'
    fix: 'Prune the top in spring, cutting just above a node. The plant branches below the cut, giving a bushier shape.'
    problemSlug: 'leggy-stretched-houseplants'
  - symptom: 'Brown crispy edges on large leaves'
    cause: 'Dry air or accumulated fertiliser salts'
    fix: 'Raise humidity slightly and flush the pot with plain water every few months.'
    problemSlug: 'brown-leaf-tips'
faqs:
  - question: 'How often should I water a rubber plant?'
    answer: 'Every 7-14 days in spring and summer and every two to three weeks in winter, watering thoroughly once the top two inches of soil are dry. Rubber plants tolerate drying out better than most ficus and dislike constantly damp soil, so check the soil rather than following a fixed schedule.'
  - question: 'Why is my rubber plant dropping leaves?'
    answer: 'Ficus elastica sheds leaves in response to change: a move to a new room, a cold draft, a heating vent switching on, or repotting. It also drops lower leaves when overwatered. Check the soil first; if it is drying properly, the cause is environmental and the plant will settle once you leave it in one stable spot.'
  - question: 'How do I make my rubber plant bushier?'
    answer: 'Prune the growing tip. In spring, cut the main stem just above a leaf node, removing several inches. The plant responds by pushing two or three new branches from below the cut. Wear gloves, because the milky sap irritates skin, and blot the cut with a damp cloth to stop the bleeding.'
  - question: 'Why is my burgundy rubber plant turning green?'
    answer: 'Insufficient light. The deep red and burgundy pigments in cultivars like Ruby, Burgundy and Abidjan develop only under bright light. In a dim spot the plant produces plain green chlorophyll-rich leaves instead. Move it within a few feet of a bright window and the new growth will colour up again.'
  - question: 'Is a rubber plant toxic to pets?'
    answer: 'Yes, mildly. The milky latex sap contains compounds that cause mouth irritation, drooling and vomiting in cats and dogs, and it can irritate human skin on contact. It is not usually dangerous but is worth keeping out of reach, and gloves are sensible when pruning.'
related:
  - fiddle-leaf-fig
  - monstera-deliciosa
  - zz-plant
featured: false
---

The rubber plant is the fiddle leaf fig's easy-going relative. Same architectural presence, same big glossy leaves, considerably more tolerance for imperfect conditions. If a fiddle leaf fig has defeated you, this is the plant to try next.

## Watering

Water thoroughly when the top two inches of soil are dry, usually every 7 to 14 days in spring and summer, and every two to three weeks in winter.

Rubber plants have a wider margin than most ficus. They handle drying out further than a fiddle leaf fig would tolerate, and they signal thirst by letting the leaves droop slightly. What they do not tolerate is sitting in wet soil, which shows up as yellowing lower leaves that drop.

Pour until water runs from the drainage holes, then let it drain fully.

## Light and leaf colour

Bright indirect light is the sweet spot. It keeps the internodes short so the plant stays compact rather than stretching, and for the coloured cultivars it develops the pigment.

Varieties like **'Ruby'**, **'Burgundy'**, **'Tineke'** and **'Abidjan'** are sold for their red, cream and near-black tones. Those pigments are light-dependent. Put a Ruby in a dim corner and within a few months it will be producing ordinary green leaves.

Some gentle direct morning sun is beneficial. Harsh afternoon sun through glass will scorch the leaves.

## Leaf drop

Ficus species are dramatic about change, and rubber plants are no exception. Common triggers:

- Moving to a different room
- Cold air from a door or single-glazed window
- A radiator or heating vent starting up for the season
- Repotting
- Sharp changes in watering frequency

The response is the same in every case: shed some leaves, then adapt. Pick a stable spot away from vents and drafts, and leave the plant there. Losing a few leaves after a move is normal; losing them continuously for months means something in the environment is still wrong.

## Pruning for shape

A rubber plant naturally grows as a single upright stem. If you want a fuller, branched shape, prune it.

1. In spring, choose where you want branching to start.
2. Cut the main stem just above a leaf node with clean, sharp secateurs.
3. **Wear gloves.** The white latex that flows out irritates skin and stains fabric.
4. Blot the cut with a damp cloth until the sap stops.
5. Within a few weeks, two or three new shoots emerge below the cut.

Save the top section as a cutting. It roots readily.

## Propagation

**Stem cuttings.** Take a section with two or three leaves. Blot the sap, remove the lowest leaf, and root either in water or directly in damp potting mix. Covering the cutting loosely with a clear bag raises humidity and speeds rooting. Expect roots in four to eight weeks.

**Air layering.** More reliable for thick, woody stems. Cut a shallow upward notch a third into the stem, wedge it open with a matchstick, wrap the area in damp sphagnum moss, and seal with clear plastic. Roots form in the moss over one to two months, after which you cut below the root ball and pot it up.

## Keeping the leaves clean

Those big leaves are dust magnets, and dust genuinely reduces the light reaching the leaf surface. Wipe both sides with a damp cloth every month, supporting the leaf with your other hand.

Avoid commercial leaf-shine sprays. They block stomata and leave a film that attracts more dust than they remove.
